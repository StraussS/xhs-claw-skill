...
class ImageDownloader:
    ...
    def download_image(self, image_url: str) -> str:
        ...
        resp = self._session.get(image_url, headers=headers, timeout=30)
        if resp.status_code != 200 and image_url.startswith("http://") and parsed.hostname and "xhscdn.com" in parsed.hostname:
            secure_url = "https://" + image_url[len("http://"):]
            resp = self._session.get(secure_url, headers=headers, timeout=30)
            image_url = secure_url if resp.status_code == 200 else image_url
